...
function _M.get_flag(remark, host)
    ...
    if (remark ~= nil) then
        ...
        remark = string.gsub(remark,'%b[]', '') 
        remark = string.gsub(remark,'test', '') 
        remark = string.gsub(remark,'TEST', '') 
        remark = string.gsub(remark,'Gaming', '') 
        remark = string.gsub(remark,'tls', '') 

        for i, v in pairs(search_table) do
            for s, t in pairs(v.regx) do
                if (string.find(string.lower(remark), string.lower(t)) ~= nil) then
...
function _M.wget(url)
    ...
    local stdout = sys.exec(
                       'wget -q --user-agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.157 Safari/537.36" -t 3 -T 10 -O- "' ..
                           url .. '"')
